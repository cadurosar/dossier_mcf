...
\begin{itemize}
	\item Indépendance vis-à-vis de la quantité de calculs et des capacités de mémoires : les phases d'apprentissage des algorithmes d'apprentissage machine sont très intenses en calculs. Aujourd'hui, ce sont les géants du numériques qui ont à disposition la plus grande puissance calculatoire. Pour mitiger ce déséquilibre, une des voies possibles est la conception d'architectures hétérogènes et d'outils puissant pour les utiliser (Section \ref{subsubsec:archis_heterogenes}).
	...
	\item Indépendance vis-à-vis des méthodes : les entreprises doivent prendre part à la révolution de l'IA en s'appropriant ses méthodes. Cet apprentissage peut passer par des échanges entre chercheurs et industriels. Dans ce cadre, j'ai rédigé en partenariat avec l'entreprise WorldCast Systems un développement de projet CIFRE qui pourra servir de base à une future candidature pour un financement de l'ANRT. Ce projet porte sur l'utilisation d'algorithme d'apprentissage machine appliquée à la maintenance prédictive (Section \ref{subsubsec:cifre}).
\end{itemize}
...
La question désormais est de savoir si les algorithmes d'apprentissage machine peuvent être utilisée pour la conception et le décodage de codes correcteurs d'erreurs. Certains travaux présentent d'ores et déja des résultats intéressants, en utilisant des réseaux de neurones profonds pour l'encodage et le décodage \cite{7926071,NIPS2018_8154}. Une autre approche est possible, présentée dans \cite{DBLP:journals/corr/abs-1901-05719}.
