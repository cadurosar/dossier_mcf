...
\end{quote}
En filigrane de cette citation et tout au long du rapport, apparaît également la problématique de l'indépendance des états, des entreprises et des citoyens. Le développement de l'IA est en effet en grande partie réalisé par un nombre très réduit d'acteurs (GAFAM\footnote{Google, Amazon, Facebook, Apple, Microsoft}) qui ont de l'avance sur les méthodes, possèdent les données, la puissance calculatoire et des moyens financiers difficiles à concurrencer. Comment agir pour contrer cette mainmise ?
Du point de vue d'un chercheur en électronique numérique, il est possible d'agir pour participer à cette indépendance.

Les phases d'apprentissage des algorithmes d'apprentissage machine sont très intenses en calculs. Aujourd'hui, ce sont les géants du numériques qui ont à disposition la majeure partie de la puissance calculatoire. Pour mitiger ce déséquilibre, une des voies possibles est la conception d'architectures hétérogènes et d'outils puissant pour les utiliser (Section \ref{subsubsec:archis_heterogenes}). Ensuite, du point de vue de l'électronique embarquée, les industriels doivent pouvoir concevoir leurs propres circuits, notamment pour s'assurer de la sécurité des données. Des méthodologies efficaces de conception électronique peuvent contribuer à cette indépendance comme les méthodologies ASIP présentées dans la Section \ref{subsubsec:asip_ia} de ce projet de recherche. Les entreprises doivent également prendre part à la révolution de l'IA en s'appropriant ses méthodes. Cet apprentissage peut passer par des échanges entre chercheurs et industriels. Dans ce cadre, j'ai rédigé en partenariat avec l'entreprise WorldCast Systems un développement de projet CIFRE qui pourra servir de base à une future candidature pour un financement de l'ANRT. Ce projet porte sur l'utilisation d'algorithmes d'apprentissage machine appliquées à la maintenance prédictive dans le domaine de la radiodiffusion (Section \ref{subsubsec:cifre}).
...
Pour des applications sur les réseaux de neurones convolutionnels, les architectures couramment utilisées sont basées sur des petits éléments de calculs dupliqués un grand nombre de fois \cite{du2015shidiannao,6292202}. Cette parallélisation est possible à l'aide des outils TCE \cite{TCEToolset}. Il sera intéressant, pour un concepteur d'architectures numériques sur cible ASIC et FPGA, de pouvoir adapter la structure de chaque élément de calcul ainsi que les espaces de mémoire qui leur sont alloués. En effet, une des problématiques de ces réseaux de neurones est la gestion de flux de données. Ces problématiques de méthodologie de conception sont d'un grand intérêt pour les industriels, qui souhaitent disposer d'outil permettant des mises sur le marché rapide de systèmes répondant à des contraintes de performance, de consommation et de surface occupée sur la puce. \'Etant rodé à la conception d'ASIP et bénéficiant de l'appui de l'équipe \og Customized Parallel Computing \fg de l'Université de Tampere, je pense pouvoir contribuer efficacement à ce domaine.

%\subsubsection{Développement d'un projet de thèse CIFRE}
...
L'art de la construction des codes correcteurs d'erreurs est un domaine de recherche actif depuis la proposition de la théorie de l'information de Claude Shannon en 1948 \cite{shannon1948mathematical}. Diverses familles de codes ont été proposées, de plus en plus complexes. Sur le plan conceptuel, les outils mathématiques et d'analyse sont de plus en plus raffinés. Sur le plan calculatoire, les traitements à réaliser au niveau du décodage sont de plus en plus lourds. Cette complexité du décodage a suivi la croissance exponentielle de la puissance de calcul des circuits électroniques (loi de Moore).
...
La question désormais est de savoir si les algorithmes d'apprentissage machine peuvent être utilisée pour la conception et le décodage de codes correcteurs d'erreurs. Certains travaux présentent d'ores et déjà des résultats intéressants, en utilisant des réseaux de neurones profonds pour l'encodage et le décodage \cite{7926071,NIPS2018_8154}. Une autre approche est possible, présentée dans \cite{DBLP:journals/corr/abs-1901-05719}.
...
Mes travaux antérieurs seront un tremplin parfait pour effectuer cette recherche. L'évaluateur de la Figure \ref{fig:ai_coding} est la partie critique du mécanisme en termes de temps de calcul. En effet, à chaque itération, une simulation Monte Carlo doit être effectuée. Or pour qu'une mesure soit fiable, un nombre raisonnable d'erreurs doit être simulé (typiquement 100). Pour des taux d'erreur trame faibles, le temps de simulation devient extrêmement long. Or le projet AFF3CT\footnote{\url{https://aff3ct.github.io}} auquel je participe très activement permet justement la simulation très haut débit de l'ensemble des différents codes correcteurs d'erreurs, et ce de manière très générique et flexible vis-à-vis des paramètres d'entrée. Cette généricité permettra, dans le mécanisme d'apprentissage représenté en Figure \ref{fig:ai_coding}, d'explorer toutes les dimensions du problèmes. En continuant sur l'exemple des codes polaires, il sera par exemple possible d'explorer tout couple (N,K), tout vecteur de bit gelés, tout motif de poinçonnage, et la majeure partie des algorithmes de décodage existant. Pour chaque algorithme de décodage, il est également possible de modifier des paramètres internes. Pour le décodage SCL (Successive Cancellation List) ces paramètres sont par exemple la profondeur de la liste, l'élagage de l'arbre de décodage, la quantification des données, le polynôme d'un éventuel CRC concaténé.

De plus, ce type d'apprentissage peut s'appliquer à d'autres éléments de la chaîne de communication, comme le codage source, la génération des formes d'ondes ou les étapes de synchronisation.
...
Ces deux UE ont pour objectif de présenter aux étudiants les algorithmes d'intelligence artificielle et de leur apprendre à réaliser des implémentations matérielles et logicielles efficaces. L'UEE3 consiste en des projets d'implémentation qui doivent être réalisées par les étudiants. Des objectifs de performance sont fixés (pourcentage de réussite) ainsi que des contraintes d'optimisation (latence, débit et surface occupée si l'implémentation est matérielle). Ayant un profil très proche de ces problématiques, je pourrai y intervenir efficacement. Ce pourra être également l'occasion d'expérimenter avec les étudiants de nouvelles techniques d'implémentation, dans le but d'intégrer recherche et enseignement.
